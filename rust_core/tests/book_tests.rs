...
fn test_orderbook_snapshot_and_sorting() {
    ...
    let bids = vec![
        (dec!(65000.0), dec!(1.5)),
        (dec!(65010.0), dec!(2.0)), // Higher price should be Best Bid
        (dec!(64990.0), dec!(5.0)),
    ];
    let asks = vec![
        (dec!(65020.0), dec!(1.0)), // Lower price should be Best Ask
        (dec!(65030.0), dec!(3.0)),
    ];

    book.apply_snapshot(100, bids, asks);

    let (best_bid_p, best_bid_q) = book.best_bid().unwrap();
    let (best_ask_p, best_ask_q) = book.best_ask().unwrap();

    assert_eq!(best_bid_p, dec!(65010.0));
    assert_eq!(best_bid_q, dec!(2.0));
    assert_eq!(best_ask_p, dec!(65020.0));
    assert_eq!(best_ask_q, dec!(1.0));
}
...
fn test_orderbook_delta_update_and_metrics() {
    ...
    let delta = DepthDelta {
        ...
        first_update_id: 101,
        final_update_id: 101,
        bids: vec![(dec!(101.0), dec!(2.0))], // New better bid
        asks: vec![(dec!(102.0), dec!(0.0)), (dec!(103.0), dec!(1.0))], // Remove 102.0, Add 103.0
        exchange_ts: Utc::now(),
        received_ts: Utc::now(),
    };
    ...
    assert_eq!(best_bid_p, dec!(101.0));
    assert_eq!(best_ask_p, dec!(103.0));
...
fn test_orderbook_sequence_gap_detection() {
    ...
    // Send gap delta (expected 101, but send 105)
    let delta = DepthDelta {
        symbol: "BTCUSDT".into(),
        first_update_id: 105,
        final_update_id: 106,
